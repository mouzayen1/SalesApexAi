...
describe('runRehash eligibility filtering', () => {
  ...
  it('filters out lenders when vehicle age exceeds maxVehicleAgeYears', () => {
    const deal = baseDeal({ vehicleYear: 2008 }); // 18 years old (2026 - 2008 = 18)
    const result = runRehash(deal);
    expect(result.allCandidates.length).toBe(0);
  });
...
describe('State-specific tax credit', () => {
  it('applies trade-in tax credit in Texas', () => {
    const dealTX = baseDeal({ state: 'TX', tradeAllowance: 5000, tradePayoff: 0 });
    const dealCA = baseDeal({ state: 'CA', tradeAllowance: 5000, tradePayoff: 0 });
  ...
  it('applies trade-in tax credit in Florida', () => {
    const dealFL = baseDeal({ state: 'FL', tradeAllowance: 5000, tradePayoff: 0 });
    const dealNV = baseDeal({ state: 'NV', tradeAllowance: 5000, tradePayoff: 0 });
  ...
  it('applies trade-in tax credit in Arizona', () => {
    const dealAZ = baseDeal({ state: 'AZ', tradeAllowance: 5000, tradePayoff: 0 });
    const dealCA = baseDeal({ state: 'CA', tradeAllowance: 5000, tradePayoff: 0 });

    const resultAZ = runRehash(dealAZ);
    const resultCA = runRehash(dealCA);

    // AZ has trade-in tax credit, CA does not
    expect(resultAZ.bestDeal?.amountFinanced).toBeLessThan(
      resultCA.bestDeal?.amountFinanced ?? 0
    );
  });

  it('does not apply tax credit when no trade equity', () => {
    const dealTX = baseDeal({ state: 'TX', tradeAllowance: 0, tradePayoff: 0 });
    const dealCA = baseDeal({ state: 'CA', tradeAllowance: 0, tradePayoff: 0 });

    const resultTX = runRehash(dealTX);
    const resultCA = runRehash(dealCA);

    // Without trade equity, both should have same amount financed
    expect(resultTX.bestDeal?.amountFinanced).toBe(resultCA.bestDeal?.amountFinanced);
  });

  it('does not apply tax credit when trade has negative equity', () => {
    const dealTX = baseDeal({ state: 'TX', tradeAllowance: 2000, tradePayoff: 5000 });
    const dealCA = baseDeal({ state: 'CA', tradeAllowance: 2000, tradePayoff: 5000 });

    const resultTX = runRehash(dealTX);
    const resultCA = runRehash(dealCA);

    // With negative equity, no tax credit applies
    expect(resultTX.bestDeal?.amountFinanced).toBe(resultCA.bestDeal?.amountFinanced);
  });
});
...
describe('APR and Reserve calculation', () => {
  it('contract rate is between buy rate and max markup', () => {
    ...
    // Contract APR should be reasonable (between buy and max)
    expect(result.bestDeal?.apr).toBeGreaterThan(15);
    expect(result.bestDeal?.apr).toBeLessThan(25);
  });

  it('includes dealer reserve info in adjustments', () => {
    const deal = baseDeal();
    const result = runRehash(deal);

    // Should have adjustments that include dealer reserve
    const hasReserveInfo = result.bestDeal?.adjustments.some(
      adj => adj.includes('Dealer reserve')
    );
    expect(hasReserveInfo).toBe(true);
  });
  ...
  it('includes buy rate info in adjustments', () => {
    ...
    // Should have adjustments that include buy rate
    const hasBuyRateInfo = result.bestDeal?.adjustments.some(
      adj => adj.includes('buy rate')
    );
    expect(hasBuyRateInfo).toBe(true);
  });

  it('dealer profit includes reserve', () => {
    const deal = baseDeal();
    const result = runRehash(deal);

    // Dealer profit should be positive and include reserve
    expect(result.bestDeal?.dealerProfit).toBeGreaterThan(0);
  });
});
